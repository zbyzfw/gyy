...
import logging
import datetime
from tools.infer.predict3 import detect_roi3
...
# number_e = sys.argv[5]
img_path = './photo/zhijie'
ini_path = 'photo/zhiini'
rotate = '3'
type_name = '1'
number_e = '5'
#20版三相
fp = open('wulian_meter.json', encoding='utf-8')
meter_data = json.load(fp)
meter1 = meter_data["直接式"]
meter2 = meter_data["互感式"]
...
def detect_lcd(path, config, child1):
    ...
    for i in range(1, len(cf.sections()) + 1):
        top_x = int(config.get(f"Location{i}", "topX"))
        top_y = int(config.get(f"Location{i}", "topY"))
        width = int(config.get(f"Location{i}", "width"))
        height = int(config.get(f"Location{i}", "height"))
    try:
    ...
    if results:
        ...
        if type_name == '1':
            ...
            # print(''.join(results_no_points))
            # print('总字数：',len(''.join(results_no_points)))
            ...
            for i, res in enumerate(meter1[:-1]):
                ...
                if i == 0:
                    ...
                    if res not in results_no_points:
                        set_meter.append(res)
            if len(''.join(results_no_points)) < 107:
                set_meter = ['缺字']
            ...
            if len(''.join(results_no_points)) < 104:
                set_meter = ['缺字']
