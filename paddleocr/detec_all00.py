...
def detect_lcd(path, config, child1):
    ...
    if range_d.find('2') != -1:
        if type_name == '2' or type_name == '4':
            if rotate=='3':
                roi1=img[0:h1,top_x + width:w1]
                roi2=img[0:h1,0:top_x]
            if  rotate=='0':
                roi1 = img[0:top_y, 0:w1]
                roi2 = img[top_y+height+100:w1, 0:h1]
            if  rotate=='1':
                roi1 = img[0:h1, 0:top_x]
                roi2 = img[0:h1, top_x+width+100:w1]

        else:
            if rotate=='3':
                roi1 = img[top_y  + height:w1, top_x:top_x + width]
                roi2 = img[0:h1, 0:top_x]
            if rotate=='0':
                roi1 = img[top_y:top_y + height,top_x+width :h1]
                roi2 = img[top_y + height:w1,0:h1]
            if  rotate=='1':
