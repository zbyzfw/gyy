...
# number_e = sys.argv[5]
img_path = './photo/img11'
ini_path = './photo/ini11'
rotate = '3'
type_name = '1'
range_d='12'
number_e = '2'
...
def detect_lcd(path, config, child1):
    ...
    if range_d.find('2') != -1:
        ...
        top_x2 = int(config.get(f"Location3", "topX"))
        top_y2 = int(config.get(f"Location3", "topY"))
        width2 = int(config.get(f"Location3", "width"))
        height2 = int(config.get(f"Location3", "height"))
        try:
            roi1= img[top_y1:top_y1 + height1, top_x1:top_x1 + width1, ]
            roi2 = img[top_y2:top_y2 + height2, top_x2:top_x2 + width2, ]
        except Exception as e:
            tree41 = E.BAR_CODE(f"{path} ini error")
            tree42 = E.CHECK_RESULT(
                E.REGION_TYPE("1"),
        ...
        set_meter = []
        results1 = detect_roi(roi1)
        results2 = detect_roi(roi2)
        print(results1)
        print(results2)
        cv2.imshow("3", roi1)
        cv2.waitKey(1000)
